...
依存管理は **uv**。`uv sync` で `.python-version`(3.13)の環境が整う。開発依存は
PEP 735 の dependency-group `dev` として宣言してあり、uv は `dev` を既定グループと
して扱うため `uv sync` / `uv run` で**自動的に入る**(`--extra` も `--no-sync` も不要)。
